document.addEventListener("DOMContentLoaded", () => {
    ...
    function loadAllSongs() {
        ...
        const promises = allFolders.map(folder => 
            fetch(`/data/${folder}.json`) // ✅ 替换为 JSON 文件
                .then(res => res.json())
                .then(data => {
                    if (!data.error) {
    ...
    // 加载文件夹和文件
    fetch("/data/folders.json") // ✅ 替换为 JSON 文件
        .then(res => res.json())
        .then(folders => {
    ...
    function loadLyrics(songName, folder) {
        ...
        const possibleLyricsFiles = [
            `/music/${folder}/${baseName}.lrc`,
            `/music/${folder}/lyrics/${baseName}.lrc`,
            `/lyrics/${folder}/${baseName}.lrc`,
            `/music/lyrics/${folder}/${baseName}.lrc`
        ];
        
        let lyrics = '';
        for (const file of possibleLyricsFiles) {
            fetch(file)
              .then(res => {
